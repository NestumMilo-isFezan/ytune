...
from ytune.app import TrackItem, YTune
...
def mock_deps(mocker):
    ...
    return {"api": mock_api.return_value, "player": mock_player, "db": mock_db}
...
async def test_auto_mix_on_track_end(mock_deps):
    ...
    async with app.run_test() as pilot:
        ...
        app.current_track = Track(
            videoId="current", title="Now playing", artist="Now artist"
        )

        # Simulate track end by calling action_skip_track directly
        # (handle_track_end is designed to be called from a background thread;
        #  action_skip_track is the actual logic we want to test)
        app.action_skip_track()

        # Give worker time to fetch and play
...
async def test_search_results_mapping(mock_deps):
    app = YTune()
    async with app.run_test() as pilot:
        # Post a SearchResultsReady message to trigger the display handler
        tracks = [Track(videoId="vid1", title="Song 1", artist="Artist 1")]
        app.post_message(YTune.SearchResultsReady(tracks))
